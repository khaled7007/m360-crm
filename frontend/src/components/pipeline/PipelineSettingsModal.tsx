...
export function PipelineSettingsModal({ onClose }: { onClose: () => void }) {
  const { config, updateStage, updateStatus, reset } = usePipelineConfig();

  // local draft copies so edits don't flicker on every keystroke
  const [stageDraft, setStageDraft] = useState(() =>
    config.stages.map((s) => ({ ...s }))
  );
  const [statusDraft, setStatusDraft] = useState(() =>
    Object.entries(config.statusConfig).map(([key, v]) => ({ key, ...v }))
  );

  const handleSave = () => {
    stageDraft.forEach((s) => updateStage(s.id, { label: s.label, labelEn: s.labelEn, headerBg: s.headerBg }));
    statusDraft.forEach((s) => updateStatus(s.key, { label: s.label, color: s.color, bg: s.bg }));
    onClose();
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" dir="rtl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-stone-800">إعدادات خط سير المبيعات</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          {/* ── Pipeline stages ── */}
          <section>
            <h3 className="text-sm font-semibold text-stone-700 mb-3">أعمدة خط سير المبيعات</h3>
          ...
          {/* ── Status badges ── */}
          <section>
            <h3 className="text-sm font-semibold text-stone-700 mb-3">تسميات حالة الطلبات (الوسوم)</h3>
            <div className="space-y-2">
              {statusDraft.map((s, i) => (
                <div key={s.key} className="flex items-center gap-3 bg-stone-50 rounded-lg px-4 py-3 border border-stone-100">
                  {/* Preview badge */}
                  <span
                    className="text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap shrink-0 w-32 text-center"
                  ...
                  <div className="flex flex-col items-center gap-1">
                    <label className="text-xs text-stone-400">لون النص</label>
                    <input
                      type="color"
                      value={s.color}
                      onChange={(e) => {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-stone-200">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={handleSave}
              className="px-5 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition text-sm font-medium"
            >
              حفظ التغييرات
            </button>
          </div>
